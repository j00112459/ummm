...
const keywordMap = {
  random: ['한식', '분식', '식당', '밥집', '음식점', '정식', '도시락', '치킨', '피자', '버거', '샐러드', '샌드위치', '카페밥'],
...
async function fetchMenuFromTourAPI(placeName, lat, lng) {
  ...
  try {
    // 위치 기반으로 근처 음식점 검색
    const searchRes = await axios.get('https://apis.data.go.kr/B551011/KorService1/locationBasedList1', {
      params: {
    ...
    // 상세 메뉴 정보 조회
    const detailRes = await axios.get('https://apis.data.go.kr/B551011/KorService1/detailIntro1', {
      params: {
        serviceKey: process.env.PUBLIC_DATA_API_KEY,
...
app.get('/api/recommend', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: 'Kakao API 호출 오류가 발생했습니다.' });
  }

  if (!restaurants || restaurants.length === 0) {
    return res.status(404).json({ error: '근처에 조건에 맞는 식당을 찾지 못했어요.' });
  }
  ...
  // TourAPI 메뉴 조회 (없으면 null)
  const menuInfo = await fetchMenuFromTourAPI(restaurant.place_name, lat, lng);
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `당신은 점심 식당을 추천해주는 도우미입니다.
아래 규칙을 반드시 지켜서 2~3문장의 자연스러운 추천 문장을 작성하세요.
- 과장 금지 (예: "리뷰가 좋습니다", "사람들이 극찬합니다" 금지)
- 제공된 정보만 사용 (없는 정보 창작 금지)
- 리뷰를 본 것처럼 말하기 금지
- 카테고리, 거리, 도보 시간, 선택 유형을 자연스럽게 언급`
        },
        {
          role: 'user',
